...
(async () => {
    ...
    // Simple process-level lock to avoid multiple concurrent Chromium instances
    const lockFile = '/tmp/playwright_js.lock';
    const maxWait = parseInt(process.env.LOCK_WAIT_MS || '60000'); // default 60s
    const startWait = Date.now();
    const fsExists = (p) => {
        try { return fs.existsSync(p); } catch(e){return false}
    };
    while (fsExists(lockFile)) {
        // check if owning PID is alive
        try {
            const ownerPid = parseInt(fs.readFileSync(lockFile, 'utf8') || '0');
        ...
        if (Date.now() - startWait > maxWait) {
            console.error('❌ Playwright lock wait timeout');
            process.exit(2);
        }
    ...
    console.error(`❌ Playwright 執行失敗: ${error.message}`);
    process.exitCode = 1;
    } finally {
    // remove lockfile
    try { fs.unlinkSync('/tmp/playwright_js.lock'); } catch(e){}
    }
})();
